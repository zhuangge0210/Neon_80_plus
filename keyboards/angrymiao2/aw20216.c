...
bool AW20216_read(pin_t cs_pin, uint8_t page, uint8_t reg, uint8_t* data, uint8_t len) {
    static uint8_t s_spi_transfer_buffer[2] = {0};
    ...
    s_spi_transfer_buffer[0] = (AWINIC_ID | page | AW_READ);
    s_spi_transfer_buffer[1] = reg;
    ...
    if (spi_receive(data, len) != SPI_STATUS_SUCCESS) {
        spi_stop();
        return false;
    }
...
static void AW20216_init_pwm_clock(pin_t cs_pin) {
    AW20216_write_register(cs_pin, AW_PAGE_FUNCTION, AW_REG_PWM_CLOCK, 0x40);
}
...
void AW20216_Default_Setting(pin_t cs_pin, pin_t en_pin) {
    // setPinOutput(en_pin);
    // writePinHigh(en_pin);
